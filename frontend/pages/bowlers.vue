<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
      <div class="space-y-2">
        <div class="inline-flex items-center gap-2 px-3 py-1 bg-ipl-blue/10 text-ipl-blue rounded-full text-xs font-bold uppercase tracking-widest">
          <CricketBallIcon class="w-3 h-3" />
          Bowling Intelligence
        </div>
        <h1 class="text-4xl font-black text-slate-900 tracking-tight">The <span class="bg-gradient-to-r from-ipl-blue to-purple-600 bg-clip-text text-transparent">Bowling Masters</span></h1>
    ...
    <div class="bg-gradient-to-br from-white via-ipl-blue/5 to-purple-50 rounded-3xl border border-ipl-blue/10 shadow-xl overflow-hidden relative">
      <!-- Decorative Elements -->
      <div class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-ipl-blue/20 to-purple-400/20 rounded-full -mr-12 -mt-12 animate-pulse-slow"></div>
      
      <div class="px-6 py-4 border-b border-ipl-blue/10 bg-gradient-to-r from-ipl-blue/5 to-purple-100/50 flex items-center justify-between relative z-10">
        <div class="flex items-center gap-3">
          <div class="p-2 bg-ipl-blue/10 rounded-xl">
            <CricketBallIcon class="w-4 h-4 text-ipl-blue" />
          </div>
          <h2 class="text-sm font-bold text-ipl-blue uppercase tracking-widest">🎯 Top Wicket Takers (All Time)</h2>
        </div>
      ...
      <div class="mobile-table-container">
        <table class="w-full text-left min-w-[600px]">
          <thead>
            <tr class="bg-gradient-to-r from-ipl-blue/5 to-purple-100/30 text-xs font-black text-slate-700 uppercase tracking-widest border-b border-ipl-blue/20">
              ...
              <th class="px-3 sm:px-6 py-4 text-center min-w-[100px] text-red-700">Economy</th>
              <th class="px-3 sm:px-6 py-4 text-right min-w-[100px] text-ipl-blue">Total Wickets</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-50">
            <tr v-for="(player, idx) in bowlers" :key="idx" class="hover:bg-gradient-to-r hover:from-ipl-blue/5 hover:to-purple-50 transition-all duration-200 group border-b border-slate-100/50">
              ...
              <td class="px-3 sm:px-6 py-3 sm:py-4 text-center">
                <span class="px-3 py-1.5 bg-gradient-to-r from-red-100 to-pink-100 text-red-800 text-sm font-black rounded-xl border border-red-200 shadow-sm whitespace-nowrap">
                  🎯 {{ player.economy }} ECO
                </span>
              </td>
              <td class="px-3 sm:px-6 py-3 sm:py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <span class="text-xl sm:text-2xl font-black bg-gradient-to-r from-ipl-blue to-purple-600 bg-clip-text text-transparent whitespace-nowrap">{{ player.wickets }}</span>
